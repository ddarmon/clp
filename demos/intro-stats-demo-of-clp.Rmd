...
```{r}
risk <- risk.conf(x = c(15, 5), n = c(30, 30))

risk$pcurve(1) # Test for no difference in risk

risk <- risk.conf(x = c(15, 5), n = c(30, 30), log = 'x') # Log-scale for confidence curve
```
...
plot.cconf(conf.lm$betas$`(Intercept)`, xlab = '(Intercept)')
plot.cconf(conf.lm$betas$waiting, xlab = 'waiting')
```
