...
for (var.name in names(coef(mod.lm))){
  ...
  cur.conf <- conf.out$betas[[var.name]]
...
```
```{r}
plot.dconf(conf.out$betas$age)
plot.cconf(conf.out$betas$age)

plot.dconf(conf.out$betas$height)
plot.cconf(conf.out$betas$height)
```
