...
The `clp` package constructs confidence functions for the most of the parameters covered in a typical introductory statistics or research methods course, including:
...
There are helper functions to plot the confidence distribution, density, curve, etc., from an output of a `clp` function. These helper functions have the names `plot.*()`. For example, to plot the \(P\)-curve giving the two-sided \(P\)-values, we use:

```{r}
plot.pcurve(conf.mu.lowcarb, xlab = 'Mean Weight Change (lb)')
```

To plot the \(S\)-curve, we use:

```{r}
plot.scurve(conf.mu.lowcarb, xlab = 'Mean Weight Change (lb)')
```
...
`t.conf()` computes the approximate confidence functions for the difference between two population means from independent samples using Welch's two-sample \(t\)-test:
...
The two-sided \(P\)-value for the hypothesis that \(\mu_{\text{LF}} - \mu_{\text{LC}} = \delta\) is generated using `plot.pcurve()`:
...
The confidence distributions for the individual population risks \(p_{C}\) and \(p_{T}\) using the mid \(P\)-value for the right-sided test,
...
And we extract the marginal 95% confidence curves for each rate using their respective quantile functions:
...
We plot the confidence curve with a 99.9% confidence curve manually using `plot.cconf()`:

```{r}
plot.cconf(conf.diff.props, xlab = 'p[C] - p[R]', conf.level = 0.999)
```
...
```{r}
plot.pconf(conf.rel.risk, compl = TRUE,
           xlab = 'p[T]/p[C]')
```

Finally, we can compute a 90% confidence distribution for \(\rho\) using the confidence quantile function:
